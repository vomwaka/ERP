...
<div class="row">
	<div class="col-lg-5">
        ...
        @endif
		 ...
		 <form onsubmit="return Validate(this);" method="POST" action="{{ URL::to('requestlicensekey')}}" id="requestform" accept-charset="UTF-8">
...
<?php }else if(Organization::binToStr($part[0]) != $organization->name || Organization::binToStr($part[5]) != 'Approved' || Organization::binToStr($part[1]) != $organization->license_code){?>
<h3><span style="color:red">Invalid license key! </span></h3>
<?php }else if(file_exists(public_path().'/uploads/license txts/payroll/'.Organization::binToStr($part[0]).' payroll license '.str_replace("-", "", Organization::binToStr($part[4])).'.license')){?>
...
<table class="table table-bordered table-hover">
    ...
    <tr>
     ...
     @else
      <td> Licensed Employees </td><td><?php echo Organization::binToStr($part[3]) ?></td>
      @endif
     ...
     @else
      <td> End Support Period </td><td><?php echo Organization::binToStr($part[4]) ?></td>
      @endif
...
<form method="POST" action="{{ URL::to('activatedproducts/updatelicense/payroll')}}" id="requestform" accept-charset="UTF-8"> 
<input class="form-control" placeholder="" type="hidden" name="pcode" id="pcode" value="{{Organization::binToStr($part[2])}}">
<input class="form-control" placeholder="" type="hidden" name="employees" id="employees" value="{{Organization::binToStr($part[3])}}">
<input class="form-control" placeholder="" type="hidden" name="period" id="period" value="{{Organization::binToStr($part[4])}}">
